...
Planned but not implemented in-repo: Docker, microservices, CI lint/test, full category module.
...
│   │   ├── user.module.ts
│   │   ├── product.module.ts
│   │   ├── shoppingCard.module.ts  # ShoppingCart model (filename typo)
│   │   └── category.module.ts      # Empty schema stub
│   ├── controllers/v1/
│   │   ├── authenticatiin.contoller.ts  # Auth (filename typos)
│   │   ├── user.controller.ts
...
│   │   ├── transporterMail.ts
│   │   └── uploamulter.ts        # Multer disk storage
...
| POST   | `/user/reset-password` | `authenticationHandler`, `validate(ResetPasswordSchema)` | `restPassword` |
