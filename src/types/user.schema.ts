...
export const userSchema = z.object({
  ...
  authProvider: z.enum(["local" , "auth0"]).openapi({description: "auth provider of the user"}),
  ...
  favoritsProduct: z.array(z.string()).openapi({description: "favoritsProduct of the user"}),
  pymemtId: z.string().openapi({description: "pymemtId of the user"}),
  shoppingCartId: z.string().openapi({description: "shoppingCartId of the user"}),
})
